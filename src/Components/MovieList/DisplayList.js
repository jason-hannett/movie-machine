    ...
    <div className="main-page">
      <div className="display-card">
        <section className="poster-title">
          ...
          />
          <p className='title' onClick={() => props.history.push(`/movie/${props.movie.id}`)}>{props.movie.title}</p>
        </section>
        <section className="rating-add">
          <img className='star' src={Star} alt='star'/>
          <p>{props.movie.vote_average}</p>
          <button onClick={() => handleAddLikedMovie(props.id)} className='add-button'>+ Watchlist</button>
        </section>
      </div>
    </div>
  );
